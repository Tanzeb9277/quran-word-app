...
export class DistractorGenerator {
  ...
  /**
   * Normalize a transliteration string for phonetic comparison
   * @param {string} str - Transliteration string
   * @returns {string} - Normalized string
   */
  static normalizeTransliteration(str) {
    if (!str) return ''
  ...
  static normalizeTranslation(str) {
    if (!str) return ''
    return str
      .toLowerCase()
      .replace(/[.,;:!?'"()\[\]{}]/g, '')
  ...
  /**
   * Compute phonetic similarity between two transliterations
   * @param {string} a - First transliteration
   * @param {string} b - Second transliteration
   * @returns {number} - Similarity score (0.0 to 1.0)
   */
  static transliterationSimilarity(a, b) {
    const normA = this.normalizeTransliteration(a)
    const normB = this.normalizeTransliteration(b)
  ...
  static async getPhoneticDistractors(correctWord, count = 3) {
    // Skip if correct word has no transliteration
    if (!correctWord.transliteration) return []

    const len = correctWord.transliteration.length

    // Fetch candidates with similar transliteration length from DB
    const candidates = await sql`
      SELECT arabic_text, transliteration, translation,
             location, root_arabic, surah_number, id,
             grammar, image_url, root_latin, tags
      FROM words
      WHERE transliteration IS NOT NULL
        AND transliteration != ''
        AND LENGTH(transliteration) BETWEEN ${len - 3} AND ${len + 3}
        AND location != ${correctWord.location}
        AND root_arabic IS DISTINCT FROM ${correctWord.root_arabic}
      LIMIT 300
    `

    // Score by transliteration similarity, keep 0.35–0.75 range
    // Too high (>0.75) = too confusingly similar
    // Too low (<0.35) = obviously different, no phonetic challenge
    const scored = candidates
      .map(c => ({
        ...c,
        phoneticScore: this.transliterationSimilarity(
          correctWord.transliteration,
          c.transliteration
        )
      }))
      .filter(c => c.phoneticScore >= 0.35 && c.phoneticScore <= 0.75)
      .sort((a, b) => b.phoneticScore - a.phoneticScore)

    // Shuffle top candidates (take top 30, shuffle, pick count)
    return this.shuffleArray(scored.slice(0, 30)).slice(0, count)
  }
  ...
  static async getSmartDistractors(correctWord, count = 3, options = {}) {
    ...
    const usedTranslations = new Set([
      this.normalizeTranslation(correctWord.translation),
      ...avoidTranslations.map(t => this.normalizeTranslation(t))
    ])
    ...
    const filterValidDistractors = (distractors) => {
      return distractors.filter(d => {
        ...
        if (usedTranslations.has(this.normalizeTranslation(d.translation))) return false
        // Check containment and similarity
        return this.isValidDistractor(d, correctWord)
      })
    }

    // Phonetic distractors first (primary — transliteration similarity)
    const phoneticDistractors = await this.getPhoneticDistractors(correctWord, count * 3)
    const filteredPhonetic = filterValidDistractors(phoneticDistractors)
    allDistractors.push(...filteredPhonetic)
    filteredPhonetic.forEach(d => usedTranslations.add(this.normalizeTranslation(d.translation)))
    ...
    if (includeSemantic) {
      ...
      filteredSemantic.forEach(d => usedTranslations.add(this.normalizeTranslation(d.translation)))
    ...
    if (includeSurah && allDistractors.length < count) {
      ...
      newDistractors.forEach(d => usedTranslations.add(this.normalizeTranslation(d.translation)))
    ...
    const uniqueDistractors = allDistractors.filter((distractor, index, self) =>
      index === self.findIndex(d => this.normalizeTranslation(d.translation) === this.normalizeTranslation(distractor.translation))
    )
