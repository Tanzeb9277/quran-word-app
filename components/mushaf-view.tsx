...
export function MushafView({ currentPage, onPageChange }: MushafViewProps) {
  ...
  const fetchPageData = async (page: number) => {
    try {
      setLoading(true)
      const response = await fetch(`/api/mushaf/page/${page}`)
      if (!response.ok) {
        throw new Error(`Failed to fetch page ${page}`)
      }
      const result = await response.json()
      if (!result.success) {
        throw new Error(result.error || `Failed to fetch page ${page}`)
      }
  ...
  // Remove hardcoded data - this will be replaced by fetched data
  const _unused_hardcoded_data: PageData = {
    page_number: 567,
    info: {
      name: "Quran Complex V1 ( 1405 print )",
      number_of_pages: 604,
  ...
  return (
    <TooltipProvider>
      <div className="h-screen flex flex-col bg-background overflow-hidden">
        {/* Header */}
        <header className="shrink-0 h-12 border-b border-border bg-card text-card-foreground flex items-center justify-between px-3">
          <Button variant="ghost" size="sm" onClick={() => onPageChange(Math.max(1, currentPage - 1))} className="h-8 w-8 p-0">
            <ChevronLeft className="h-4 w-4" />
          </Button>

          <div className="text-center">
            <h1 className="text-sm font-semibold text-foreground">Page {pageData.page_number}</h1>
            <p className="text-xs text-muted-foreground">Page {pageData.page_number} of {pageData.info.number_of_pages}</p>
          </div>

          <Button variant="ghost" size="sm" onClick={() => onPageChange(Math.min(pageData.info.number_of_pages, currentPage + 1))} className="h-8 w-8 p-0">
            <ChevronRight className="h-4 w-4" />
          </Button>
        </header>

        <div className="overflow-y-auto" style={{ height: selectedWord ? '85vh' : '92.5vh' }}>
          ...
          <div className="w-full max-w-3xl mx-auto px-4 py-3">
            ...
            <div className="flex flex-col gap-3">
              {pageData.lines.map((line) => (
                <div key={line.line_number} className="w-full flex justify-between items-baseline gap-2" dir="rtl">
                  {line.words.map((word) => (
                    <Tooltip
                    ...
                    >
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className={`font-arabic transition-colors hover:text-primary inline-flex items-baseline ${
                        ...
                        >
                          ...
                          {word.is_last_word_of_verse && (
                            <span className="mr-1 inline-flex items-center justify-center rounded-full border border-current align-middle" style={{ 
        ...
        {selectedWord && (
          <div className="shrink-0 border-t border-border/50 bg-muted/30 p-3">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <span className="font-arabic text-lg shrink-0">{selectedWord.arabic_text}</span>
